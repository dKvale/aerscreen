...
#' @param results_file File to save AERSCREEN modeling results to. 
...
write_aerscreen <- function(data            = NULL,
  ...
  inp_text <- paste0(inp_text, 
                     ...
                     "**              ",
                     if(sum(sapply(unlist(bu[ ,c("bld_height", 
                                                 "width_x", 
                                                 "length_y", 
                                                 "bld_rotation", 
                                                 "angle_from_source", 
                                                 "dist_from_source")]), function(x) aermod::is_valid(x, 1))) > 5) {
                       paste0(receptors::fw("Y", 6),
                       receptors::fw(bu$bld_height, 10),
                       receptors::fw(max(bu$width_x, bu$length_y, na.rm  = TRUE), 10),
                       receptors::fw(min(bu$width_x, bu$length_y, na.rm  = TRUE), 10),
                       receptors::fw(ifelse(max(bu$width_x, bu$length_y) == bu$width_x, bu$bld_rotation + 90, bu$bld_rotation), 10),
                       receptors::fw(bu$angle_from_source, 10),
                       bu$dist_from_source,
                       collapse = "")
                     } else "N",
                     "\n\n", collapse = "")
...
#235.00  315.00   0.5   10.000    0    0   0.1000   0.0100   1.0020  "NA"                                                                                                                                                                 
  ...
  inp_text <- paste0(inp_text,
                     ...
                     "**                   ",
                     "N            0.0         0.0     0     0        ",
                     receptors::fw(co$far_receptor, 16), 
                     "0.00         N",
                     "\n\n", collapse = "")
  ...
  if(aermod::is_valid(so$urban_pop, 1) & so$urban_pop < 100) {
  ...
  inp_text <- paste0(inp_text,
                      ...
                      "**                      ",
                      "M     ",
                      receptors::fw(ifelse(aermod::is_valid(so$urban_pop, 1), "U", "R"), 8),
                      receptors::fw(so$urban_pop, 17),
                      receptors::fw(ifelse(aermod::is_valid(co$near_receptor, 1), co$near_receptor, "1"), 13),
                      receptors::fw(ifelse(aermod::is_valid(co$flagpole_height, 1), "Y", "N"), 10),
                      as.numeric(co$flagpole_height),
                     "\n\n", collapse = "")
 ...
 inp_text <- paste0(inp_text,
  ...
  ## OUTPUT section 
  inp_text <- paste0(inp_text, 
                     '** OUTPUT FILE "', gsub("[.]out", "", results_file), '.out"',
                     "\n\n", collapse = "")
